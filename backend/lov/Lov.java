...
public class Lov extends View
{
	...
	public void selectRow(ViewRow row)
	{
		...
		{
	    	if (owner.hasColumn(columnNames[i]) || owner.view.isFreeColumnsEnabled())
	    	{
	    		Object value = null;
	    		if (row != null)
	    		{
	...
	public void selectRowSetOtherValues(String col,ViewRow row)
	{
		setCurrentRow(row);
		String[] columnNames = getColumnNames();
		for (int i = 0; i < columnNames.length;i++)
		{
	    	if ((owner.hasColumn(columnNames[i]) || owner.view.isFreeColumnsEnabled()) && !col.equals(columnNames[i]))
	    	{
	    		Object value = null;
	    		if (row != null)
	    		{
	...
	{
        if (getColumnTypes()[getColumnIndex(col)].equals(BigDecimal.class))
        {
        	if  (val != null)
        	{
	        	try
