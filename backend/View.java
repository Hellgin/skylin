...
public class View 
{
				    ...
				    try 
				    {
				       ...
				       // ArrayList<Integer> newBlobColumnIndex = new ArrayList<Integer>();
				        while (rs.next()) 
				        {
				        	...
				        	{
				        		if (columnTypes[i] == java.sql.Timestamp.class)
				        		{
				        			Object date = rs.getObject(i+1);
				        			if (date != null && date instanceof oracle.sql.TIMESTAMP)
				        			{
				        				row.values.put(columnNames[i],((TIMESTAMP) date).timestampValue());
				        			}
				        			else
				        			{
				        				row.values.put(columnNames[i],date);
				        			}
				        		}
				        		else if (columnTypes[i] == SkylinBlob.class)
				        		{
				        			ByteArrayOutputStream b = new ByteArrayOutputStream();
				        		    InputStream in = rs.getBinaryStream(i+1);
				        		    SkylinBlob sb = (SkylinBlob)row.getValueInternal(columnNames[i]);
				        		    if (in != null)
				        		    {
				        		    ...
				        		    //row.values[i] = sb;
				        		    row.values.put(columnNames[i],sb);
				        		    b.close();
				        		}
				        		else
				        		{
				        			//row.values[i] = rs.getObject(i+1);
				        			 row.values.put(columnNames[i],rs.getObject(i+1));
				        		}
				        	}
				        	/*
	...
	public void setKey(String... keyColumns)
	{
		if (freeColumns)
		{
			throw new SkylinException(getAM(),"setKey not yet supported in free columns mode");
		}
		...
		keyFields = new String[keyColumns.length];
		loop:
			...
			//if (cacheForThisClass == null || noGenCache)
			{
				try
				{
						...
						for (int i = 0; i < count; i++)
						{
							...
							if (columnTypes[i] == oracle.sql.TIMESTAMP.class)
							{
								columnTypes[i] = java.sql.Timestamp.class;
							}
							else if (Blob.class.isAssignableFrom(columnTypes[i]))
							{
		...
		for (int i = 0; i < bindingNames.size();i++)
		{
			...
			for (int j = 0; j < row.view.columnNames.length;j++)
			{
				...
				if (bindingNames.get(i).equals(row.view.columnNames[j]))
				{
					...
					bindingValues.set(i, row.values.get(bindingNames.get(i)));
	...
	public void setRowType(Class type)
	{
		...
		if (columnNames.length == 0 && !freeColumns)
		{
	...
	public void setEditableColumns(String... c)
	{
		if (freeColumns)
		{
			throw new SkylinException(getAM(),"setEditableColumns not yet supported in free columns mode");
		}
		if (columnNames.length == 0)
		{
	...
	public void setNotEditableColumns(String... c)
	{
		if (freeColumns)
		{
			throw new SkylinException(getAM(),"setNotEditableColumns not yet supported in free columns mode");
		}
		if (columnNames.length == 0)
		{
	...
	*/
	...
	public final List<ViewRow> getRows()
	{
		return rows;
	}
...
/*
	...
	*/
	...
	public View getLinkedView(Class viewClass){return getLinkedView(viewClass,new String[]{});}
	...
	public View getLinkedView(String instanceName)
	{
		...
		return cur.getLinkedView(instanceName);
	}
	...
	{
		System.out.println("addColumn: todo");
	}
	...
	{
		//return columnTypes;
		Class[] ret = new Class[columnTypes.length];
		for (int i = 0; i < ret.length;i++)
		{
			ret[i] = getColumnType(i);
		}
		return ret;
	}
	...
	public void enableFreeColumns()
	{
		freeColumns  = true;
		if (setRowTypeLater != null)
		{
			rowType = setRowTypeLater;
			setRowTypeLater = null;
		}
	}
	
	public boolean isFreeColumnsEnabled() 
	{
		return freeColumns;
	}
	
	public void fromJ(J json)
	{
		fromJ(json,null);
	}
	
	public void fromJ(String json)
	{
		fromJ(J.fromString(json),null);
	}
	
	public void fromJ(String json,JToViewConfig config)
	{
	...
	public final Class getColumnType(int i)
	{
		return getColumnType(columnNames[i]);
	}
	...
	public void setCurrentCol(String s)
	{
		currentCol = s;
	}
	
	public boolean isColSelectable(String col)
	{
		return false;
	}
	
	public List<ViewRow> getRowsForTable(String layout)
	{
		return getRows();
	}
	
	
	public final List<ViewRow> getRowsBetweenForTable(int start, int end, String layout)
	{
		List<ViewRow> rows = getRowsForTable(layout);
	...
	public int getSizeForTable(String layout)
	{
		return getRowsForTable(layout).size();
	}

	public void moveCol(String from, String to)
	{
		
	}

	public void validateAll() throws SQLException, ValidationException {
		for (ViewRow r:getRows())
		{
			r.validateAll();
		}
	}
